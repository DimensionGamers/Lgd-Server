...
bool MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        sLog->outError(LOG_SQL, "Could not initialize Mysql connection to database `%s`", m_connectionInfo.database.c_str());
        return false;
    }
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            sLog->outInfo(LOG_SQL, "MySQL client library: %s", mysql_get_client_info());
            sLog->outInfo(LOG_SQL, "MySQL server ver: %s ", mysql_get_server_info(m_Mysql));
        ...
        sLog->outInfo(LOG_SQL, "Connected to MySQL database at %s", m_connectionInfo.host.c_str());
    ...
    {
        sLog->outError(LOG_SQL, "Could not connect to MySQL database at %s: %s\n", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            sLog->outInfo(LOG_SQL, "SQL: %s", sql);
            sLog->outError(LOG_SQL, "[%u] %s", lErrno, mysql_error(m_Mysql));
        ...
        //else
        //    sLog->outDebug(LOG_SQL, "[%u ms] SQL: %s", getMSTimeDiff(_s, getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    uint32 index = stmt->m_index;
    {
        ...
        WPAssert(m_mStmt);            // Can only be null if preparation failed, server side error or bad query
       ...
       //sLog->outDebug(LOG_SQL, "[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString(m_queries[index].first).c_str());
...
bool MySQLConnection::_Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64* pRowCount, uint32* pFieldCount)
{
...

    {
        ...
        //else
        //    sLog->outDebug(LOG_SQL, "[%u ms] SQL: %s", getMSTimeDiff(_s, GetTickCount()), sql);
...
bool MySQLConnection::ExecuteTransaction(SQLTransaction& transaction, SQLTransactionCallBack& callback)
{
    ...
    for (itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    sLog->outWarn(LOG_SQL, "Transaction aborted. %u queries not executed.", (uint32)queries.size());
                    RollbackTransaction();
...
{
    WPAssert(index < m_stmts.size());
    MySQLPreparedStatement* ret = m_stmts[index];
    if (!ret)
        sLog->outError(LOG_SQL, "Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        sLog->outError(LOG_SQL, "In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql);
        sLog->outError(LOG_SQL, "%s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            sLog->outError(LOG_SQL, "In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql);
            sLog->outError(LOG_SQL, "%s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            ...
            {
                sLog->outInfo(LOG_SQL, "Connection to the MySQL server is active.");
                if (oldThreadId != mysql_thread_id(GetHandle()))
                    sLog->outInfo(LOG_SQL, "Successfully reconnected to %s @%s:%s (%s).",
        ...
        case ER_NO_SUCH_TABLE:
            sLog->outError(LOG_SQL, "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        case ER_PARSE_ERROR:
            sLog->outError(LOG_SQL, "Error while parsing SQL. Core fix required.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        default:
            sLog->outError(LOG_SQL, "Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
            return false;
    }
}