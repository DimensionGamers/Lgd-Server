...
bool SQLQueryHolder::SetQuery(size_t index, const char *sql)
{
    ...
    {
        sLog->outError(LOG_SQL, "Query index (%u) out of range (size: %u) for query: %s", uint32(index), (uint32)m_queries.size(), sql);
        return false;
    }
...
bool SQLQueryHolder::SetPQuery(size_t index, const char *format, ...)
{
    if (!format)
    {
        sLog->outError(LOG_SQL, "Query (index: %u) is empty.", uint32(index));
        return false;
    }
    ...
    if (res == -1)
    {
        sLog->outError(LOG_SQL, "SQL Query truncated (and not execute) for format: %s", format);
        return false;
    }
...
bool SQLQueryHolder::SetPreparedQuery(size_t index, PreparedStatement* stmt)
{
    if (m_queries.size() <= index)
    {
        sLog->outError(LOG_SQL, "Query index (%u) out of range (size: %u) for prepared statement", uint32(index), (uint32)m_queries.size());
        return false;
    }
