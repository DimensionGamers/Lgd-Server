...
#define SafeAssert(assertion, errmsg) { if (!(assertion)) { sLog->outError(LOG_CRASH, "\n%s:%i in %s ERROR:\n  %s\n", __FILE__, __LINE__, __FUNCTION__, (char *)errmsg); return; } }
#define SafeRAssert(assertion, errmsg, RETURN) { if (!(assertion)) { sLog->outError(LOG_CRASH, "\n%s:%i in %s ERROR:\n  %s\n", __FILE__, __LINE__, __FUNCTION__, (char *)errmsg); return RETURN; } }
